...
class SerialCommunicate:
  ...
  def try_open_serial(self):
        """Hàm kiểm tra và mở cổng serial nếu có thể"""
        try:
            # Kiểm tra cổng serial có tồn tại không
            if self.RSport not in [port.device for port in serial.tools.list_ports.comports()]:
                print(f"Port {self.RSport} không tồn tại.")
                return

            # Mở cổng serial
            self.serRS = serial.Serial(port=self.RSport, baudrate=self.RSbaudrate)
            print(f"Đã mở cổng {self.RSport}")
        
        except serial.SerialException as e:
            print(f"Lỗi khi mở cổng serial {self.RSport}: {e}")
